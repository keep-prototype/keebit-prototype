...
export const GolfZone = ({ zoneId }) => {
  ...
  return (
    <>
      ...
      <figure
        id="timeRange"
        className={`relative h-24 border rounded-2xl transition-all ${
          isRepair ? 'border-main-brown' : ''
        } ${isUse.current ? 'bg-green-light' : ''} ${
          !isRepair && !isUse.current
            ? 'border-main-orange text-main-orange'
            : ''
        } ${zoneId === selectedZone ? 'bg-main-orange' : ''}
      `}
        onClick={handleZoneClick}
      >
        <p
          className={`pt-2 pl-2 font-bold text-xs ${
            zoneId === selectedZone ? 'text-black' : ''
          }`}
        >
          {zoneId}번
        </p>
        <p
          className={`absolute top-1/2 left-1/2 -translate-1/2 w-full text-center text-sm font-bold ${
            zoneId === selectedZone ? 'text-black' : ''
          }`}
        >
        ...
        <p
          className={`absolute bottom-2 w-full text-center text-xs ${
            zoneId === selectedZone ? 'text-black' : ''
          }`}
        >
